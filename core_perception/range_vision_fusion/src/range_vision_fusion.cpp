  ...
  const autoware_msgs::DetectedObjectArray::ConstPtr &in_range_detections)
{
  ...
  std::vector<std::vector<size_t> > vision_range_assignments(in_vision_detections->objects.size());   //存储图像内点云目标的size
  std::vector<bool> used_vision_detections(in_vision_detections->objects.size(), false);    //创建size大小的bool类型数组，初始为false
  std::vector<long> vision_range_closest(in_vision_detections->objects.size());             //最近目标

    //mmg 2023-12-08
  std::string result_file_path_ = "/usr/file/catkin_autoware/result/range_vision_det/range_vision.txt";
  std::ofstream outputfile(result_file_path_, std::ofstream::out | std::ofstream::app);

  for (size_t i = 0; i < in_vision_detections->objects.size(); i++)     //相机探测到的目标
  {
    auto vision_object = in_vision_detections->objects[i];

    cv::Rect vision_rect(vision_object.x, vision_object.y, vision_object.width, vision_object.height);
    ...
    {
      double current_distance = GetDistanceToObject(range_in_cv.objects[j]);  //求目标距离

      cv::Rect range_rect = ProjectDetectionToRect(range_in_cv.objects[j]);   //将目标映射到图像上，找到图像中目标的边框，返回一个图像上的四边形
      int range_rect_area = range_rect.area();

      cv::Rect overlap = range_rect & vision_rect;             //求检测框的交集
      if ((overlap.area() > range_rect_area * overlap_threshold_)   //IoU满足阈值条件，雷达检测的目标与相机检测目标为同一物体
        ...
        )
      {
        ...
        range_in_cv.objects[j].image_frame = vision_object.image_frame;   //帧ID一致
        //mmg 2023-12-08
        ...
        //mmg 2023-12-08
        if (current_distance < closest_distance)      //找到最近距离
        {
          closest_index = j;
  ...
  {
    if (!used_vision_detections[i])     //未满足IoU阈值的对象，没有使用过的相机检测目标
    {
...
ROSRangeVisionFusionApp::SyncedDetectionsCallback(        //时间同步
  ...
  const autoware_msgs::DetectedObjectArray::ConstPtr &in_range_detections)
{
  ROS_INFO("access to the sync function!");
  ...
  if (nullptr == in_vision_detections     //均为空
  ...
  if (nullptr == in_vision_detections     //图像检测为空，雷达检测不为空
      && nullptr != in_range_detections
      && !in_range_detections->objects.empty())
  {
    publisher_fused_objects_.publish(in_range_detections);  //发布雷达检测的话题
    empty_frames_++;
    return;
  }
  ...
  {
    for(int i = 0; i < in_vision_detections->objects.size(); ++i){
      std::cout << "camera_det: x: " << in_vision_detections->objects[i].x
                << ",y: " << in_vision_detections->objects[i].y 
                << ", dimensions.x: " << in_vision_detections->objects[i].dimensions.x 
                << ", dimensions.y: " << in_vision_detections->objects[i].dimensions.y << std::endl;
    }
    // publisher_fused_objects_.publish(in_vision_detections);   //发布检测相机的话题
    empty_frames_++;
    return;
  }
...
void
ROSRangeVisionFusionApp::VisionDetectionsCallback(          //相机检测话题
  const autoware_msgs::DetectedObjectArray::ConstPtr &in_vision_detections)
{
  if (!processing_ && !in_vision_detections->objects.empty())   //还未处理，且不为空
  {
    processing_ = true;
...
void
ROSRangeVisionFusionApp::RangeDetectionsCallback(           //雷达检测话题
  const autoware_msgs::DetectedObjectArray::ConstPtr &in_range_detections)
{
...
ROSRangeVisionFusionApp::InitializeROSIo(ros::NodeHandle &in_private_handle)
{
  ...
  in_private_handle.param<std::string>("detected_objects_range", detected_objects_range, "/detection/objects"); 
  ROS_INFO("[%s] detected_objects_range: %s", __APP_NAME__, detected_objects_range.c_str());  

  in_private_handle.param<std::string>("detected_objects_vision", detected_objects_vision, "/detection/image_detector/objects");      //相机检测发布的话题
  ROS_INFO("[%s] detected_objects_vision: %s", __APP_NAME__, detected_objects_vision.c_str());

  // in_private_handle.param<std::string>("camera_info_src", camera_info_src, "/camera_info"); //相机内参
  ...
  // YAML::Node car_dimensions = YAML::Load(min_car_dimensions);   //加载gtbox的尺寸
  // YAML::Node person_dimensions = YAML::Load(min_person_dimensions);
  // YAML::Node truck_dimensions = YAML::Load(min_truck_dimensions);
  // car_dimensions = [3, 2, 2]
  // person_dimensions = [1, 2, 1];
  // truck_dimensions = [4, 2, 2];
  ...
  //                                                      1,
  //                                                      &ROSRangeVisionFusionApp::IntrinsicsCallback, this); //订阅相机内参话题
  ...
  image_frame_id_ = "camera_color_left";
  camera_info_ok_ = true;
  ...
  std::cout << "lidar_to_camera_R: " << lidar_camera_R << std::endl;
  std::cout << "lidar_to_camera_T: " << lidar_camera_T << std::endl;
  ...
  q.normalize();
  Eigen::Vector3d t(lidar_camera_T.at<double>(0, 0), lidar_camera_T.at<double>(0, 1), lidar_camera_T.at<double>(0, 2));
  ...
  if (!sync_topics)     //没有同步时间，将两个topic进行时间同步
  {
    detections_range_subscriber_ = in_private_handle.subscribe(detected_objects_vision,     //订阅相机话题
                                                               1,
                                                               &ROSRangeVisionFusionApp::VisionDetectionsCallback,
                                                               this);

    detections_vision_subscriber_ = in_private_handle.subscribe(detected_objects_range,     //订阅雷达检测话题
                                                                1,
  ...
  { //消息过滤器message_filters类似一个消息缓存，当消息到达消息过滤器的时候，可能并不会立即输出，而是在稍后的时间点里满足一定条件下输出。 
    ...
    detections_synchronizer_->registerCallback(boost::bind(&ROSRangeVisionFusionApp::SyncedDetectionsCallback, this, _1, _2));  //_1, _2, 表示第一个参数和第二个参数的位置;表示将该位置留着后面传入实参
  }

  publisher_fused_objects_ = node_handle_.advertise<autoware_msgs::DetectedObjectArray>(fused_topic_str, 1);  //创建融合后的话题并发布
  publisher_image_objects_ = node_handle_.advertise<autoware_msgs::DetectedObjectArray>("/detection/image_detector/objects", 1);
