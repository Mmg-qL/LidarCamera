  ...
  private_nh_("~")
{
  ...
  private_nh_.param<double>("prevent_explosion_threshold", prevent_explosion_threshold_, 1500); //mmg 2023-12-09 1000->1500
  private_nh_.param<double>("merge_distance_threshold", merge_distance_threshold_, 0.7);  //mmg 2023-12-09 0.5->0.7
  ...
  if (is_benchmark_)
  {
    ...
    result_file_path_ = "/usr/file/catkin_autoware/result/imm_ukf_pda/benchmark_results.txt";
    std::remove(result_file_path_.c_str());
  }

  //mmg 2023-12-05
  std::string pkg_loc = ros::package::getPath("imm_ukf_pda_track");
  ...
  std::cout << "imm_ukf_pda camera_matrix: " << camera_instrinsics_ << std::endl;
  //读取in_message的畸变系数参数存入distortion_coefficients_变量
  fs["distortion_coefficients"] >> distortion_coefficients_;   //畸变系数
  std::cout << "imm_ukf_pda distortion_coefficients: " << distortion_coefficients_ << std::endl;
  ...
  std::cout << "imm_ukf_pda lidar_to_camera_R: " << lidar_camera_R << std::endl;
  std::cout << "imm_ukf_pda lidar_to_camera_T: " << lidar_camera_T << std::endl;
                                      ...
                                      std::vector<bool>& matching_vec)
{
  ...
  if (exists_smallest_nis_object)
  {
    ...
    {
      object_vec.push_back(target.object_); //object_vec存储量测
    }
  }
}
...
void ImmUkfPda::initTracker(const autoware_msgs::DetectedObjectArray& input, double timestamp)
{
  for (size_t i = 0; i < input.objects.size(); i++)
  {
    ...
    //mmg 2023-12-11
    double pyaw = tf::getYaw(input.objects[i].pose.orientation);
    Eigen::VectorXd init_meas = Eigen::VectorXd(2);
    init_meas << px, py;

    UKF ukf;
    ukf.initialize(init_meas, timestamp, target_id_, pyaw);
...
//mmg 2023-12-5
cv::Point2i 
ImmUkfPda::ProjectPoint(const cv::Point3f &in_point)
{
...
//mmg 2023-12-5
cv::Rect 
ImmUkfPda::ProjectDetectionToRect(const autoware_msgs::DetectedObject &in_detection)
{
...
void ImmUkfPda::dumpResultText(autoware_msgs::DetectedObjectArray& detected_objects)
{
  ...
  for (size_t i = 0; i < detected_objects.objects.size(); i++)
  {
    ...
    //         << std::to_string(10*detected_objects.objects[i].pose.position.z) << ","
    //         << std::to_string(10*detected_objects.objects[i].dimensions.x) << ","
    //         << std::to_string(10*detected_objects.objects[i].dimensions.y) << ","
    //         << std::to_string(10*detected_objects.objects[i].dimensions.z) << ","
    //         << "-1" << ","
    //         << "-1" << ","
    //         << "-1" << ","
    //         << "-1" << ","
    //         << "\n";
      ...
      // //图像坐标系
      // cv::Rect range_rect = ProjectDetectionToRect(detected_objects.objects[i]);
      // outputfile << detected_objects.objects[i].header.stamp << "," 
      //         << frame_count_ << "," 
      //         << detected_objects.objects[i].id << " "
      //         << range_rect.x << ","
      //         << range_rect.y << ","
      //         << range_rect.width << ","
      //         << range_rect.height << ","
